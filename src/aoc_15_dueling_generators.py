...
def generator(seed, factor, mod_filter=1):
    while True:
        value = (seed * factor) % 2147483647
        if value % mod_filter == 0:
            yield value
        seed = value


def compare_values(x_1, x_2):
    def bin_string(h):
        return bin(h)[2:].zfill(32)[-16:]

    return bin_string(x_1) == bin_string(x_2)


def judge(gen_a, gen_b, sample_size):
    return sum(1 for _ in range(sample_size) if compare_values(next(gen_a), next(gen_b)))


if __name__ == '__main__':
    a = generator(679, 16807)
    b = generator(771, 48271)

    equal_count = judge(a, b, sample_size=40_000_000)
    print(f'Result for Part 1: {equal_count}')

    a = generator(679, 16807, 4)
    b = generator(771, 48271, 8)

    equal_count = judge(a, b, sample_size=5_000_000)
    print(f'Result for Part 2: {equal_count}')
