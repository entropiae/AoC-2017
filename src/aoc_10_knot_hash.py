...
from src.utils import wrap_index, read_first_line, get_slice, set_slice, chunks
...
def parse_keys_part_1(raw_puzzle_input):
    return [int(k) for k in raw_puzzle_input.split(',')]


def parse_keys_part_2(raw_puzzle_input):
    keys = [ord(k) for k in raw_puzzle_input]
...
def knot_hash(plain, keys):
    ...
    dense_hash = [reduce(lambda x, y: x ^ y, chunk) for chunk in chunks(sparse_hash, 16)]
    return ''.join("%0.2x" % x for x in dense_hash)


def hash_round(plain, keys, rounds=1):
    current_position = 0
    skip_size = 0
    ...
    for key in keys:
        plain, current_position, skip_size = step(plain, current_position, skip_size, key)
    return plain


def step(plain, current_position, skip_size, key):
    cypher = plain[:]
    cypher_segment = reversed(get_slice(cypher, current_position, current_position + key))
    cypher = set_slice(cypher, cypher_segment, current_position, current_position + key)
    return cypher, wrap_index(current_position + key + skip_size, cypher), skip_size + 1


if __name__ == '__main__':
    plain = list(range(256))
    raw_keys = read_first_line('knot_hash.txt')

    crypted = hash_round(plain, parse_keys_part_1(raw_keys))
    print(f'Result for Part 1: {crypted[0] * crypted[1]}')

    computed_hash = knot_hash(plain, parse_keys_part_2(raw_keys))
    print(f'Result for Part 2: {computed_hash}')
